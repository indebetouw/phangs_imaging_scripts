...
pipedir = '/nexus/posix0/MIA-astro-env/eschinner/shared/ALMA/pipeline/'

# Location of the master key. Set this to the master key that points
# to all of the keys for your project.

key_file = pipedir + 'alma-centers_keys/master_key.txt'
...
##this_uvh.set_interf_configs(only=['12mext+12m+7m'])
...
##this_pph.set_interf_configs(only=['12mext+12m+7m'])
##this_pph.set_feather_configs(only=['12mext+12m+7m+tp'])
...
if do_imaging:
    ...
    this_imh = ImagingChunkedHandler('ngc1097_sdint', '12m+7m', 'co21', this_kh,
                                     chunksize=40,imaging_method='sdintimaging'
                                     )
    # this_imh.task_make_dirty_image()
    
    # Loop over each chunk to completion
    this_imh.run_imaging(do_all=True, chunk_num=None)
    # When running per chunk, combining into final cubes is a separate call
    this_imh.task_complete_gather_into_cubes(root_name='all', imaging_method='sdintimaging')
    # (Optional) generate FITS cubes
    this_imh.task_export_to_fits(tag=None, imaging_method='sdintimaging')
...
if do_postprocess:
    this_pph.loop_postprocess(do_prep=True, do_feather=False,imaging_method='sdintimaging',
